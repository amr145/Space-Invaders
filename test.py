...
async def show_loading_screen():
    # Initialize Turtle in a separate window
    ...
    def draw_circle_midpoint(radius):
        def plot_point(x, y, center_x=0, center_y=0):
            # Plot 8 symmetric points for a circle centered at (center_x, center_y)
            t.penup()
            for px, py in [
                (x, y), (x, -y), (-x, y), (-x, -y),
                (y, x), (y, -x), (-y, x), (-y, -x)
                ...
                t.pendown()
                t.dot(2)  # Small dot for pixel-like effect
                t.penup()
    ...
    try:
        turtle.tracer(0)  # Disable animation for instant drawing
        draw_circle_midpoint(100)  # Draw circle with radius 100
        turtle.tracer(1)  # Re-enable animation
...
async def game_loop(screen, WIDTH, HEIGHT, two_players):
    ...
    try:
        ...
        bullet_img = pygame.transform.scale(bullet_img, (10, 20))  # Adjust size for laser
    except Exception:
        bullet_img = pygame.Surface((4, 10))
        bullet_img.fill(WHITE)  # Fallback to white rectangle
    ...
    powerup_speed = 1.5  # Slower speed for levels 1-3
    ...
    last_powerup_health = 100  # Track boss health for power-up drops
    ...
    while level <= max_level + 1:
        ...
        if level == 1:
            ...
            powerup_spawn_rate = float('inf')  # No time-based power-ups in boss level
            required_enemies = 0
            if not boss:
                try:
        ...
        while running:
            ...
            # Spawn power-ups for levels 1-3
            if level <= max_level:
            ...
            for bullet in bullets[:]:
                ...
                if level == 4 and boss and bullet.colliderect(boss):
                    ...
                    boss_health -= 1
                    score += 20
                    # Spawn power-ups every 20 HP lost
                    ...
                    if boss_health <= 0:
                        if boss_defeat_sound:
                            boss_defeat_sound.play()  # Play boss defeat sound
                        running = False
                        level += 1
            for i in sorted(enemies_to_remove, reverse=True):
                enemies.pop(i)
            ...
            for bullet in bullets:
                screen.blit(bullet_img, bullet)  # Draw laser image instead of rectangle
            for enemy in enemies:
    ...
    # Show loading screen before returning to main menu
